...
def load_split(split):
    ...
    videos = []
    game_list_entries = getListGames(split) # Get the list of entries
    print(f"--- Entries for split '{split}' ---")
    if not game_list_entries:
        print(f"Warning: getListGames({split}) returned an empty list.")
    for entry in game_list_entries:
        print(f"Processing entry: '{entry}'") # Print each entry
        # The problematic line:
        league, season, game = entry.replace('\\', '/').split('/')
        videos.append((league, season, game))
    print(f"--- Finished processing entries for split '{split}' ---")
    return videos
